...
public class BatchRendererUtil {
     ...
     * @return
     */
    public static function addRectangularMesh(renderer:BatchRenderer, width:int, height:int):int {
        if(width <= 0 || height <= 0) throw new ArgumentError("width and height must be greater than zero");

        var numVertices:int = width * height;
        var firstVertex:int = renderer.addVertices(numVertices);

        for(var row:int = 0; row < height - 1; ++row) {
            for(var col:int = 0; col < width - 1; ++col) {
                var v:int = firstVertex + col + row * width;

                renderer.addTriangle(v    , v     +     1, v + width);
                renderer.addTriangle(v + 1, v + width + 1, v + width);
            }
        }
     ...
     *
     * @param renderer              renderer containing the geometry
     ...
     */
    public static function getGeometryBounds(renderer:BatchRenderer, positionID:int, vertex:int = 0, numVertices:int = -1, resultRect:Rectangle = null, transformationMatrix:Matrix = null):Rectangle {
        if(resultRect == null) resultRect = new Rectangle();

        if(numVertices < 0) numVertices = renderer.vertexCount - vertex;
        ...
        if(numVertices == 0) {
            ...
            if(transformationMatrix == null) {
                for(i = 0; i < numVertices; ++i) {
                    renderer.getVertexData(vertex + i, positionID, _helperVector);
    ...
    public static function adjustUV(renderer:BatchRenderer, texture:Texture, uvID:int, vertex:int = 0, numVertices:int = -1):void {
